class Clinic < ActiveRecord::Base
  ...
  def as_json(options)
    ...
    result = {:id => self.id, :name => self.name, :address => self.address, :specialization => self.specialization, :address=> self.address||'', :latitude=> self.latitude||'', :longitude=>self.longitude||''}
  end
